...
export async function getTopUsers(): Promise<FetchResponse<TopUserWithProfile[]>> {
    ...
    if (error) 
        return {data: null, error: {message: 'Problem getting users', status: 500}};

    const ids = [...new Set(users?.map(u => u.userId))];
    const qs = encodeURIComponent(ids.join(','));

    const {data: profiles, error: profileError} = await fetchClient<Profile[]>(`/profiles/batch?ids=${qs}`, 'GET',
